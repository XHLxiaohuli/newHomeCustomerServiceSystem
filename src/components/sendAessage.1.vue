<template>
  <div class="judgment">
    <p  v-if="type!=='10000' && type!=='2000' && type!=='55535'"  
        :style="boolobj?'text-align: right; box-sizing: border-box;':'text-align: left; box-sizing: border-box;'">
        <span style="font-size:8px;color:#aaa;">{{getdate(time)}}</span>&nbsp;&nbsp;<span style="font-size:8px;color:#aaa;">{{name}}</span>
    </p>
    <div v-if="type!=='10000'" :class="boolobj ? 'sendAessage_' : 'closedAessage_'">
      <img :src="boolobj ? topUrl[1]:topUrl[0]" alt=""/>
      <div  :class="boolobj ? 'sendText' : 'sendText_c_c'" 
            v-html="showMsg"
      ></div>
    </div>
